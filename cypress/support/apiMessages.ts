export const apiMessages = {
  ...
  ProcessWorkerErrorOneOfStatutCode: (type: string, field: string, code: string, id: string, aliquot: string, value: string) => `Invalid field ${field} for ${type} (${code} / ${id} / ${aliquot}). Reason: \"${value}\" is not a valid status code. Valid values [completed, draft, inconclusive, in_progress, in_review, processing, reopened, resolved, revoked, submitted, unresolved].`,
  ProcessWorkerErrorOneOfTypeCode: (type: string, field: string, code: string, id: string, value: string) => `Invalid field ${field} for ${type} (${code} / ${id}). Reason: \"${value}\" is not a valid type code. Valid values [blood, buccal_swab, buffy_coat, dna, not_reported, rna, saliva, solid_tissue].`,
  ...
  UnmarshalStringTo: (field: string, type: string) => `json: cannot unmarshal string into Go struct field ${field} of type ${type}`,
};
