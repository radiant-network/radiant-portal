...
export const CasesTable = {
  ...
  validations: {
     ...
     * @param columnID The ID of the column to check.
     */
    shouldHaveSelectedQueryPill_TODO(dataCase: any, columnID: string) {
      switch (columnID) {
        case 'gene':
          cy.validatePillSelectedQuery('Gene Symbol', [dataCase[columnID]]);
          break;
        default:
          cy.validatePillSelectedQuery(getColumnName(tableColumns, columnID), [dataCase[columnID]]);
          break;
      }
    },
    /**
     ...
     */
    shouldShowColumnContent_TODO(columnID: string, dataCase: any) {
      CasesTable.actions.showAllColumns();
      getColumnPosition(selectorHead, tableColumns, columnID).then(position => {
        if (position !== -1) {
          switch (columnID) {
            ...
            case 'gene':
              ...
              break;
            case 'consequence':
              cy.validateTableFirstRowClass(dataCase.consequenceImpact, position);
              cy.validateTableFirstRowContent(dataCase[columnID], position);
              cy.validateTableFirstRowContent(dataCase.aa_change, position);
              break;
            case 'mane':
              cy.get(selectors.tableCell(dataCase))
                .eq(position)
                .find(CommonSelectors.maneCPath)
                .should(dataCase.maneC ? 'exist' : 'not.exist');
              cy.get(selectors.tableCell(dataCase))
                .eq(position)
                .find(CommonSelectors.maneMPath)
                .should(dataCase.maneM ? 'exist' : 'not.exist');
              cy.get(selectors.tableCell(dataCase))
                .eq(position)
                .find(CommonSelectors.manePPath)
                .should(dataCase.maneP ? 'exist' : 'not.exist');
              break;
            case 'omim':
              dataCase[columnID].forEach((value: string | RegExp) => {
                cy.validateTableFirstRowContent(value, position);
              });
