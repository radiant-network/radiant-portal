...
export const CaseEntity_Files_Filters = {
  actions: {
    /**
     ...
     * @param filterKey The filter key (e.g. 'format_code').
     */
  ...
  validations: {
    ...
    shouldChangeResultsOnApply(filterKey: string) {
      ...
      CaseEntity_Files_Filters.actions.openFilter(filterKey);
      CaseEntity_Files_Filters.actions.selectFirstOption(filterKey);
    ...
    shouldClearAllFilters(filterKey: string) {
      ...
      CaseEntity_Files_Filters.actions.openFilter(filterKey);
      CaseEntity_Files_Filters.actions.selectFirstOption(filterKey);
      CaseEntity_Files_Filters.actions.clickClearAll();
    ...
    shouldClearFilter(filterKey: string) {
      CaseEntity_Files_Filters.actions.openFilter(filterKey);
      CaseEntity_Files_Filters.actions.selectFirstOption(filterKey);
      CaseEntity_Files_Filters.actions.clickFilterClear(filterKey);
      cy.get(CommonSelectors.filterBadge(filterKey)).should('not.exist');
    },
    /**
     * Validates that the buckets returned by GET cases/{case_id}/documents/filters are rendered as options of a filter.
     ...
     * @param filterKeyA The first filter key to apply (e.g. 'format_code').
     * @param filterKeyB The second filter key to apply (e.g. 'data_type_code').
     */
    shouldRequestMultipleFilters(filterKeyA: string, filterKeyB: string) {
      ...
      CaseEntity_Files_Filters.actions.openFilter(filterKeyA);
      CaseEntity_Files_Filters.actions.selectFirstOption(filterKeyA);
      ...
      CaseEntity_Files_Filters.actions.openFilter(filterKeyB);
      CaseEntity_Files_Filters.actions.selectFirstOption(filterKeyB);
