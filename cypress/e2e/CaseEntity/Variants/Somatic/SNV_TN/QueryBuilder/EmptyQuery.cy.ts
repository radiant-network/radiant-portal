/// <reference types="cypress"/>
import 'support/commands';
import { data } from 'pom/shared/Data';
import { CaseEntity_Variants_QueryBuilder } from 'pom/pages/CaseEntity_Variants_QueryBuilder';
import { CaseEntity_Variants_SavedFilters } from 'pom/pages/CaseEntity_Variants_SavedFilters';

describe('Case Entity - Variants - Somatic - SNV (TN) - Query builder - Empty query', () => {
  const setupTest = () => {
    cy.login();
    cy.visitCaseVariantsPage(data.caseSomatic.case, data.caseSomatic.seq.seq_id, 'SNV', { cohort: 'TN' });
    CaseEntity_Variants_SavedFilters.somatic.actions.selectFilterInDropdown('Cypress_All_Variants'); // Apply a filter for a cleaner query builder
    CaseEntity_Variants_SavedFilters.somatic.actions.clickNewFilterButton(); // Clean Query Builder (empty query)
  };

  it('Build a first pill', () => {
    setupTest();
    CaseEntity_Variants_QueryBuilder.somatic.actions.buildFirstPill();

    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldHaveChromosomePill();
    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldHaveActivePillCount(1);
    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldHaveQueryCount();
    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldShowClearAll(false /*shouldExist*/);
  });

  it('Collapse and expand the query panel', () => {
    setupTest();
    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldBeCollapsed(false /*collapsed*/);

    CaseEntity_Variants_QueryBuilder.somatic.actions.toggleCollapse();
    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldBeCollapsed(true /*collapsed*/);

    CaseEntity_Variants_QueryBuilder.somatic.actions.toggleCollapse();
    CaseEntity_Variants_QueryBuilder.somatic.validations.shouldBeCollapsed(false /*collapsed*/);
  });
});
