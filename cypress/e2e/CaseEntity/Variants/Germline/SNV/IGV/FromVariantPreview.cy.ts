...
describe('Case Entity - Variants - Germline - SNV - IGV - From variant preview', () => {
  const setupTest = () => {
    cy.login();
    cy.visitCaseVariantsPage(data.case.case, data.case.seq.seq_id, 'SNV', data.variantGermline.sqon);
    CaseEntity_Variants_SNV_Table.germline.actions.selectAction(data.variantGermline, 'preview');
