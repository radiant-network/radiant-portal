...
describe('Case Entity - Variants - Germline - SNV - Query builder - One numerical pill', () => {
  const setupTest = () => {
    cy.login();
    cy.visitCaseVariantsPage(data.case.case, data.case.seq.seq_id, 'SNV');
    CaseEntity_Variants_SavedFilters.snv.actions.selectFilterInDropdown('Cypress_QB_1NumericalPill');
  };

  it('Edit a pill via its inline popup', () => {
    setupTest();
    CaseEntity_Variants_QueryBuilder.snv.actions.editPillInline();

    CaseEntity_Variants_QueryBuilder.snv.validations.shouldHaveNumericalPill(['9']);
