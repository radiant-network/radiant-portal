...
describe('Occurrences - Germline - SNV - Count - Occurrence', () => {
  ...
  sectionData.facets.forEach(facet => {
    ...
    it(`${facetName}`, () => {
      const facetData = globalData.Count.Occurrence.find((f: { field: string }) => f.field === facet.apiField);

      if (!facetData) {
        throw new Error(`Facet data for "${facet.apiField}" not found in globalData.Count.Occurrence`);
      }
