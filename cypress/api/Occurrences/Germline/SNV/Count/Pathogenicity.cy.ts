...
describe('Occurrences - Germline - SNV - Count - Pathogenicity', () => {
  ...
  sectionData.facets.forEach(facet => {
    ...
    it(`${facetName}`, () => {
      const facetData = globalData.Count.Pathogenicity.find((f: { field: string }) => f.field === facet.apiField);

      if (!facetData) {
        throw new Error(`Facet data for "${facet.apiField}" not found in globalData.Count.Pathogenicity`);
      }
