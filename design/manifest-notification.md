...
| Grouping | Two Postgres tables (PR review 2026-09-17, back to the first proposal after the team reconsidered the comma-joined text): `case_group` header (`id` identity, `tenant_code`, `name`, `created_on`, `created_by`, `UNIQUE (tenant_code, name)`; the name stays the business key, the id only keys membership) and `case_group_case` junction (`case_group_id`, `case_id`, both FK with `ON DELETE CASCADE`, index on `case_id`). A case can belong to several groups (re-analysis runs). Radiant's Airflow (`radiant-portal-pipeline`, DAG `nextflow_postprocessing_cases`) creates a group from the case ids of a post-processing run right after `register_tasks`, then triggers notify. `clin-pipelines-dags` untouched. |
...
```
Membership rows carry real FKs; existence is still validated at POST time so a 400 lists the unknown ids instead of a bare FK error. `created_by` = Keycloak `sub`. `notification_emails` comma-joined, `NULL` = no distribution list. Federation: `organization` is in `types.ViewTables`, the new column appears in the per-tenant views automatically (`FederatableColumns` reads `information_schema`); `cmd/refresh-tenants` or `VIEW_REFRESH_ON_STARTUP_ENABLED` after deploy.
...
### A2. Types
- `internal/types/case_group.go` (new): `CaseGroup` (`ID`, `TenantCode`, `Name`, `CreatedOn`, `CreatedBy`) and `CaseGroupCase` (`CaseGroupID`, `CaseID`) GORM models + `TableName()`; `CaseGroupTable` / `CaseGroupCaseTable` (Postgres only); `NormalizeCaseIDs([]int) []int` (deduplicated in client order, never nil; PR review 2026-09-17: no sort, reads come back in `case_id` order from the junction PK); `NewCaseGroupResponse(group, ids)`; `ValidateCaseGroupName`; `ErrCaseGroupNotFound`; `UnknownCaseIDsError{IDs}`; DTOs `PostCaseGroupRequest{Name string binding:"required"; CaseIDs []int binding:"required"}`, `CaseGroupResponse{Name, TenantCode, CaseIDs []int}`, `CaseGroupEmailReport{OrganizationCode, Recipients []string, CaseCount, DocumentCount, Status, Error?, Template string, Context CaseGroupEmailContext}`, `CaseGroupEmailContext{HasStat bool, AnalysisCodes []string, CaseIDs []int, ManifestFilename string}`, `NotifyCaseGroupResponse{Group CaseGroupResponse, Emails []CaseGroupEmailReport}`; status constants; read rows `CaseGroupCaseRow{CaseID, PriorityCode, AnalysisCatalogCode, DiagnosisLabCode, DiagnosisLabName}`, `CaseGroupDocumentRow{DocumentID, Name, Size, DataTypeCode, FormatCode, SubmitterSampleID, PatientID, CaseID, DiagnosisLabCode}`. Empty `case_ids: []` passes `required` (non-nil) = empty group, documented in swagger.
...
- Tests: `Test_ValidateCaseGroupName_Valid/_Empty/_PathSeparatorRejected`, `Test_UnknownCaseIDsError_Message`, `Test_NormalizeCaseIDs_DedupsKeepingOrder/_Empty_NotNil`, `Test_NewCaseGroupResponse`, `Test_SplitEmails_TrimsAndDropsEmpty`.
...
- `GetCaseGroupByName(ctx, tenantCode, name) (*types.CaseGroup, []int, error)` (nil, nil, nil when absent; ids sorted from the junction).
...
|---|---|---|---|---|---|
| CLIN-6223 | radiant-portal | `feat/clin-6223` | A1 (`case_group`), A2, A4, A6 POST/GET, A7 | migration `case_group` + `case_group_case`, types + `NormalizeCaseIDs`, repo, `POST /case_groups` (overwrite by name) + `GET /case_groups/{name}`, registries, swagger/TS client, Postman | none, first |
