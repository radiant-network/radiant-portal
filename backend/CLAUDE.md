...
**No DB default — writes set `tenant_code` explicitly.** 000013 deliberately omits a column `DEFAULT` (a default would silently route future writes to the launch tenant). Existing rows are backfilled with an explicit `UPDATE … SET tenant_code = 'radiant'`. Every Go write path sets it via the shared `types.DefaultTenantCode` constant: the worker in its `persist*` struct literals (`cmd/worker/case_validation.go`), and the API repos (`batch`, `occurrence_note`, `occurrence_flag`, interpretations) at their `Create`/`Save` choke points. **TODO(multi-tenant): once writes can read the active tenant from request context, set `tenant_code` from there instead of `DefaultTenantCode`.** (Test seed fixtures and inline test inserts list `tenant_code` explicitly too — there is no test-only default.)
