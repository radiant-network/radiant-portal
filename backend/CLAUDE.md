...
internal/
  ...
  provisioning/- Wires service.AdminDeps from env config; shared by cmd/api and cmd/create-user
...
**Worker reads are scoped by an explicit `tenantCode` argument, not by `WithTenant(ctx)`.** The batch *endpoint* is tenant-scoped, but the batch *processing* is not the same request: `POST /:tenant/…/batch` only stamps `batch.tenant_code` and returns, and `cmd/worker` picks the row up later, in a different process. That process has no HTTP request and no middleware, so nothing ever calls `types.ContextWithTenant`, and `ClaimNextBatch` claims the oldest `PENDING` row of *any* tenant. The tenant is therefore data on the claimed batch (`batch.TenantCode`), not context state — `types.TenantFromContext` returns not-ok and `postgres.WithTenant` is a no-op throughout the worker. Every repository method the batch path resolves a **natural key** with therefore takes `tenantCode` and puts `tenant_code = ?` in its `WHERE` — patient, fetus, sample, sequencing experiment, organization, project, analysis catalog, case and document — as do the `Update*` methods that match on the same keys. `task_has_document` is an instance table, so it scopes by joining `task.tenant_code`. None of these keys is unique across tenants; an unscoped lookup resolves, and sometimes writes, another tenant's row. Lookups by internal id (`GetSampleById`, `GetFetusById`, …) need no tenant — the id was already resolved within one. `batchval.BatchValidationCache` keys every natural-key map by tenant for the same reason. **A new natural-key lookup on the batch path must take `tenantCode`**; see the `*_OtherTenantRow_NotReturned` / `*_TenantIsolation_*` tests for the shape of the regression guard.
...
**Org-scoped actions resolve to the diagnosis lab of the resource's case** (`cases.diagnosis_lab_code`). Gate them with `RequireActionAt(auth, repo, action, resolver)`, picking a resolver from `internal/server/org_resolvers.go`: `OrgFromCaseParam`, `OrgFromCaseBody`, `OrgFromNoteParam`, `OrgFromDocumentParam` (backed by `AuthRepository.OrgsFor{Case,Note,Document}`). Resolving no org → generic 403. `RequireActionAtEvery` is the all-of variant for the case-batch routes: one lab per record, and the caller must hold the action at every one (a record stating no lab is resolved through the case it names). `RequireAction` is for **tenant-scoped actions only**: it passes `TenantWideOrg`, which against an org-scoped action matches only `'*'` grants and refuses every specific-org grantee. An org-scoped route whose request names no organization at all — the patient/sample/sequencing batches, a batch read — uses `RequireActionInTenant` instead, which checks the action anywhere in the tenant.

Two guards in `cmd/api` cover every `/:tenant` route: `Test_TenantRoutesAreMappedToActions` (list the route in `expectedTenantActions`, or `membershipOnlyTenantRoutes` if it is member-readable) and `Test_OrgScopedRoutesResolveTheirOrg` (an org-scoped route must be in `orgResolvedTenantRoutes`, or in `inTenantOrgActionRoutes` when its request names no org).
...
`can_read_pii` has **two** paths: a grant at the patient's own organization, or at the diagnosis lab of a case the patient is in (proband via `cases.proband_id`, family member via `family.family_member_id`). The rule lives only in the `can_read_pii` column of `views/patient.sql.tmpl`, ORing `auth.pii_grant` with `auth.pii_lab_patient`; the Ranger masks just branch on it, so changing the rule means editing the view and running `cmd/refresh-tenants`.

**Adding an `auth` view is a two-sided deploy**: the API's startup refresh creates views but never touches Ranger, so with `STARROCKS_PROXY_READ_ENABLED` on, a patient view referencing an uncovered `auth` view fails every patient read with access-denied. Hence `sr_access_auth` grants the whole `auth` database, and `cmd/refresh-tenants` must run before or with the API rollout.
...
| `KEYCLOAK_CLI_CLIENT_ID` | Public Keycloak client (device authorization grant, no secret) advertised by `GET /config` to CLI tools, together with `KEYCLOAK_HOST` / `KEYCLOAK_REALM` | radiant-client-cli |
