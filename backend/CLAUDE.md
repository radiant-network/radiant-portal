...
The remaining tenant-scoped transactional tables carry a plain `tenant_code varchar(50)` with an FK to `tenant(code)` (added by migration `000013_tenant_scope_clinical_tables.up.sql`): `document`, `family`, `family_history`, `obs_categorical`, `obs_string`, `occurrence_note`, `occurrence_flag`, `project`, `task`, `batch`, and `interpretation_germline`/`interpretation_somatic` (+ their `_history` twins, column-only, no FK — `tp_history_func` copies positionally via `$5.*`, so the column is appended last). Pure junctions (`case_has_sequencing_experiment`, `task_context`, `task_has_document`), reference/catalog dictionaries, and user-scoped tables (`saved_filter`/`user_preference`/`user_set*`) are **not** tenant-scoped. The 000013 columns are `NOT NULL DEFAULT 'radiant'` — a **transitional** default that keeps additive: API insert paths (GORM `Create`) don't set `tenant_code` yet, so the default backfills both existing rows and new writes to the launch tenant. The follow-up that scopes writes to the active tenant must set `tenant_code` explicitly and **drop these defaults**.
