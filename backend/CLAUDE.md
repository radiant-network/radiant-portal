...
- `/case_groups` — named sets of cases (one per pipeline run, the unit laboratories get notified about); `POST` creates or overwrites by name (`can_ingest_data`, checked in-tenant like the batches), `GET /{name}` reads (`can_search_case`, so the portal can turn a group into a case filter). Table `case_group` is Postgres-only (not federated), PK `(tenant_code, name)`, `case_ids` comma-joined text like `interpretation_*.classification_criterias`. No index on the ids: nothing looks groups up by case (add a GIN expression index if a screen ever does).
