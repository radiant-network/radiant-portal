...
func JoinFamilyWithAffectedStatus(tx *gorm.DB) *gorm.DB {
	return tx.Joins(fmt.Sprintf("LEFT JOIN %s %s ON %s.code = %s.affected_status_code", types.AffectedStatusTable.FederationName, types.AffectedStatusTable.Alias, types.AffectedStatusTable.Alias, types.FamilyTable.Alias))
}
