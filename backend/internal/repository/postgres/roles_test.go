...
func Test_RolesRepository_GetTenantRole_ReturnsRoleWithItsActions(t *testing.T) {
	testutils.RunTest(t, testutils.Need{Postgres: testutils.ReadPostgres}, func(t *testing.T, env *testutils.Env) {
		...
		assert.ElementsMatch(t,
			[]string{types.ActionReadPII, types.ActionInterpretVariant, types.ActionCommentVariant, types.ActionFlagVariant, types.ActionDownloadFile, types.ActionEditStatus},
			actionCodes(role))
