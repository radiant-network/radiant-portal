...
func (r *OccurrencesRepository) GetOccurrences(seqId int, userQuery types.ListQuery) ([]Occurrence, error) {
	...
	// 		LEFT JOIN (SELECT DISTINCT locus_id, sequencing_id FROM radiant_jdbc.public.interpretation_germline) i ON i.locus_id = o.locus_id AND o.seq_id = i.sequencing_id
	// WHERE o.locus_id in (
	...
	tx = r.db.Table("(germline__snv__occurrence o, germline__snv__variant v)").
		Joins("LEFT JOIN (SELECT DISTINCT locus_id, sequencing_id FROM radiant_jdbc.public.interpretation_germline) i ON i.locus_id = o.locus_id AND o.seq_id = i.sequencing_id").
		Select(columns).
