...
func Test_GetVariantInternalFrequenciesSplitBy_ByProject(t *testing.T) {
	testutils.ParallelTestWithDb(t, "simple", func(t *testing.T, db *gorm.DB) {
		...
		assert.Equal(t, 6, *(project1.Frequencies.PnAll))
		assert.Equal(t, 0.8333333333333334, *(project1.Frequencies.PfAll))
...
func Test_GetVariantInternalFrequenciesSplitBy_ByAnalysis(t *testing.T) {
	testutils.ParallelTestWithDb(t, "simple", func(t *testing.T, db *gorm.DB) {
		...
		assert.Equal(t, 7, *(WGA.Frequencies.PnAll))
		assert.Equal(t, 0.7142857142857143, *(WGA.Frequencies.PfAll))
