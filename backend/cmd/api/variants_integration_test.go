...
func assertGetVariantHeader(t *testing.T, data string, locusId int, expected string) {
	testutils.ParallelTestWithStarrocks(t, data, func(t *testing.T, db *gorm.DB) {
		repo := starrocks.NewVariantsRepository(database.StarrocksDB{DB: db})
		router := tenantRouter()
...
func assertGetVariantOverview(t *testing.T, data string, locusId int, expected string) {
	testutils.ParallelTestWithReadOnlyPostgresAndStarrocks(t, data, func(t *testing.T, srDB *gorm.DB, pgDB *gorm.DB) {
		repo := starrocks.NewVariantsRepository(database.StarrocksDB{DB: srDB})
		exomiserRepository := starrocks.NewExomiserRepository(database.StarrocksDB{DB: srDB})
		pubmedClient := &MockExternalClient{}
		interpretationRepository := postgres.NewInterpretationsRepository(database.PostgresDB{DB: pgDB}, pubmedClient)
		router := tenantRouter()
...
func Test_GetVariantOverview(t *testing.T) {
	expected := `{"aa_change":"p.Arg19His", "cadd_phred":0.1, "cadd_score":0.1, "clinvar": ["Benign", "Pathogenic"], "fathmm_pred":"T", "fathmm_score":0.1, "gnomad_loeuf":0.1, "gnomad_pli":0.1, "gnomad_v3_af":0.001, "interpretation_classification_counts":{"benign":1, "likelyPathogenic":1, "pathogenic":1}, "locus":"locus1", "is_canonical":true, "is_mane_plus":false, "is_mane_select":true, "lrt_pred":"U", "lrt_score":0.01, "omim_conditions": [{"inheritance_code": ["AD"], "panel": "Noonan syndrome 7", "omim_phenotype_id": "613706"}, {"inheritance_code": ["AD"], "panel":"LEOPARD syndrome 3", "omim_phenotype_id":"613707"}], "germline_pc_wgs":3, "germline_pf_wgs":0.99, "picked_consequences":["csq10"], "polyphen2_hvar_pred":"D", "polyphen2_hvar_score":0.991, "rsnumber":"rs111111111", "revel_score":0.1, "sift_pred":"T", "sift_score":0.1, "spliceai_ds":0.1, "spliceai_type":["AG"], "symbol":"BRAF", "transcript_id":"T001", "vep_impact":"MODIFIER"}`
...
func Test_GetVariantOverview_With_ExomiserACMGClassificationCounts(t *testing.T) {
	expected := `{"aa_change":"p.Arg19His", "cadd_phred":0.1, "cadd_score":0.1, "is_canonical":true, "clinvar": ["Benign", "Pathogenic"], "fathmm_pred":"T", "fathmm_score":0.1, "gnomad_loeuf":0.1, "gnomad_pli":0.1, "gnomad_v3_af":0.001, "interpretation_classification_counts":{"benign":1, "likelyPathogenic":1, "pathogenic":1}, "locus":"locus1", "is_mane_plus":false, "is_mane_select":true, "omim_conditions": [{"inheritance_code": ["AD"], "panel": "Noonan syndrome 7", "omim_phenotype_id": "613706"}, {"inheritance_code": ["AD"], "panel":"LEOPARD syndrome 3", "omim_phenotype_id":"613707"}], "germline_pc_wgs":3, "germline_pf_wgs":0.99, "picked_consequences":["csq10"], "revel_score":0.1, "rsnumber":"rs111111111", "sift_pred":"T", "sift_score":0.1, "spliceai_ds":0.1, "spliceai_type":["AG"], "symbol":"BRAF", "transcript_id":"T001", "vep_impact":"impact1","exomiser_acmg_classification_counts":{"Pathogenic":2,"VUS":1}}`
...
func assertGetGermlineVariantConditions(t *testing.T, data string, locusId int, panelType string, filter string, expected string) {
	testutils.ParallelTestWithStarrocks(t, data, func(t *testing.T, db *gorm.DB) {
		repo := starrocks.NewGenePanelsRepository(database.StarrocksDB{DB: db})
		router := tenantRouter()
		...
		req, _ := http.NewRequest("GET", fmt.Sprintf("/radiant/variants/germline/%d/conditions/%s%s", locusId, panelType, filterParam), bytes.NewBuffer([]byte("{}")))
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
...
func Test_GetGermlineVariantConditions_Clinvar(t *testing.T) {
	testutils.ParallelTestWithStarrocks(t, "clinvar", func(t *testing.T, db *gorm.DB) {
		repo := starrocks.NewClinvarRCVRepository(database.StarrocksDB{DB: db})
		router := tenantRouter()
		...
		expected := `{"clinvar_id":"123456","conditions":[{"locus_id":"1000","clinvar_id":"123456","accession":"RCV000001","clinical_significance":["Pathogenic"],"date_last_evaluated":"2025-01-01T00:00:00Z","submission_count":1,"review_status":"criteria_provided","review_status_stars":4,"version":1,"traits":["Trait1","Trait2"]},{"locus_id":"1000","clinvar_id":"123456","accession":"RCV000001","clinical_significance":["Pathogenic"],"date_last_evaluated":"2024-01-01T00:00:00Z","submission_count":1,"review_status":"criteria_provided","review_status_stars":4,"version":1,"traits":["Trait1"]}]}`
		assert.Equal(t, http.StatusOK, w.Code)
		assert.JSONEq(t, expected, w.Body.String())
	})
}
...
func assertGetGermlineVariantExternalFrequencies(t *testing.T, data string, locusId int, expected string) {
	testutils.ParallelTestWithStarrocks(t, data, func(t *testing.T, srDB *gorm.DB) {
		repo := starrocks.NewVariantsRepository(database.StarrocksDB{DB: srDB})
		router := tenantRouter()
