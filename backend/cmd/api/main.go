...
func setupRouter(dbStarrocks *gorm.DB, dbPostgres *gorm.DB) *gin.Engine {
	...
	orgsFromCaseIDsBody := server.OrgsFromCaseIDsBody(repoAuth)
	...
	casesGroup.POST("/assignment_candidates", server.RequireActionAtEvery(auth, repoAuth, types.ActionEditCase, orgsFromCaseIDsBody), server.ListCaseAssignmentCandidatesHandler(repoCaseAssignments, repoAuth))
