...
func Test_validateUpdateCaseRecord_Success(t *testing.T) {
	...
	rec, err := validateUpdateCaseRecord(t.Context(), ctx, cache, validCaseUpdate(), 0, map[FetusKey]struct{}{}, types.DefaultTenantCode)
...
func Test_validateUpdateCaseRecord_UnknownProject(t *testing.T) {
	...
	rec, err := validateUpdateCaseRecord(t.Context(), ctx, cache, update, 0, map[FetusKey]struct{}{}, types.DefaultTenantCode)
	assert.NoError(t, err)
