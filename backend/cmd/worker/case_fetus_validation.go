...
// persistFetuses reconciles the case's fetuses with the payload, attaching each to the case's
// proband as mother. A submitter id already on the case is updated **in place** so its row id — and
// therefore sample.fetus_id and the sequencing data behind it — survives; an unknown one is created.
// Family rows and observations are always inserted: the caller has cleared the previous ones.
// existing is nil on the create path, where every entry is new.
