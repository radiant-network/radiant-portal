...
Code should read for itself. Add a comment **only** for a genuinely tricky or non-obvious point — an injection guard, a load-bearing operation order, a subtle SQL expression, a security caveat (e.g. "a Ranger `table` policy does not reach a view"). Do **not** write a doc comment on every exported name, and do not narrate what the code plainly says (`// Create mode.`, `// loop over tenants`).
