...
export default function MetricSection({ data }: MetricSectionProps) {
  ...
  const genotypeQualityValue = (
    <span className="inline-flex gap-1 items-center">
      ...
      {data?.genotype_quality ? thousandNumberFormat(data.genotype_quality) : '-'}
    </span>
  );
  ...
  return (
    <DetailSection title={t('occurrence_expand.metrics.title')}>
      ...
      <DetailItem
        title={t('common.filters.labels.ad_alt')}
        value={data?.ad_alt ? thousandNumberFormat(data.ad_alt) : '-'}
      />
      <DetailItem
        title={t('common.filters.labels.ad_total')}
        value={data?.ad_total ? thousandNumberFormat(data.ad_total) : '-'}
      />
