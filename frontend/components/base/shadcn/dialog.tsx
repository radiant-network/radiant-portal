...
const dialogVariants = tv({
  slots: {
    base: 'fixed left-[50%] top-[50%] z-50 grid w-full max-h-[96vh] translate-x-[-50%] translate-y-[-50%] border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-lg outline-none',
    close:
  ...
  return (
    <DialogPortal>
      ...
      <DialogContext.Provider value={{ variant, size }}>
        <DialogPrimitive.Content data-cy="modal" className={style.base({ className })} {...props}>
          {children}
