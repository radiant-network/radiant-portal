...
import { isBooleanFilter, isRangeFilter } from '@/components/model/query-builder-core';
...
function QueryPillLabelOperator({ valueFilter, operator, className, ...props }: QueryPillLabelProps) {
  ...
  return (
    <div className={cn('flex items-center', className)} {...props}>
      {(showLabels || isBooleanFilter(valueFilter) || isRangeFilter(valueFilter)) && (
        <>
          <QueryPillLabelContainer>{dict.queryPill.facet(valueFilter.content.field)}</QueryPillLabelContainer>
...
export function QueryPillLabelContainer({ className, children, ...props }: React.HTMLAttributes<HTMLSpanElement>) {
  return (
    <span className={cn('ml-[4px] mr-[2px] text-xs font-medium', className)} {...props}>
      {children}
    </span>
  );
}
