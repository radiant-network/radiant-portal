...
import { createQueryBuilder, CoreQueryBuilderProps, QueryBuilderState, QueryBuilderInstance } from '../query-builder';
...
let mockOnQueryCreate: Mock<any>;
let mockOnQueryUpdate: Mock<any>;
let mockOnQueryDelete: Mock<any>;
let mockOnQuerySelectChange: Mock<any>;
let mockOnStateChange: Mock<any>;
...
describe('QueryBuilder Core', () => {
  beforeEach(() => {
    mockOnQueryCreate = vitest.fn();
    mockOnQueryUpdate = vitest.fn();
    mockOnQueryDelete = vitest.fn();
    mockOnStateChange = vitest.fn();
  ...
  it('should create a new query', () => {
    ...
    expect(mockOnQueryCreate).toHaveBeenCalledWith(expectedOnCreatedHaveBeenCalledWith);
  ...
  it('should create a new query with both indexes when combining', () => {
    ...
    expect(state.queries.find(q => q.id === mockUUID)?.content).toEqual([0, 1]);
    expect(mockOnQuerySelectChange).toHaveBeenCalledTimes(1);
    expect(mockOnQuerySelectChange).toHaveBeenCalledWith([]);
    expect(mockOnQueryCreate).toHaveBeenCalledTimes(1);
  ...
  it('should change combine operator of a query recursively', () => {
    ...
    expect(state.queries.find(q => q.id === '1')?.op).toBe(BooleanOperators.Or);
