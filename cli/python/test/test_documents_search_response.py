# coding: utf-8

"""
    Radiant API

    This is the API for Radiant data platform.

    The version of the OpenAPI document: 1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.documents_search_response import DocumentsSearchResponse

class TestDocumentsSearchResponse(unittest.TestCase):
    """DocumentsSearchResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DocumentsSearchResponse:
        """Test DocumentsSearchResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DocumentsSearchResponse`
        """
        model = DocumentsSearchResponse()
        if include_optional:
            return DocumentsSearchResponse(
                count = 56,
                list = [
                    openapi_client.models.document_result.DocumentResult(
                        case_id = 56, 
                        created_on = '', 
                        data_type_code = '', 
                        diagnosis_lab_code = '', 
                        diagnosis_lab_name = '', 
                        document_id = 56, 
                        format_code = '', 
                        hash = '', 
                        name = '', 
                        patient_id = 56, 
                        relationship_to_proband_code = '', 
                        run_alias = '', 
                        seq_id = 56, 
                        size = 56, 
                        submitter_sample_id = '', 
                        task_id = 56, )
                    ]
            )
        else:
            return DocumentsSearchResponse(
                count = 56,
                list = [
                    openapi_client.models.document_result.DocumentResult(
                        case_id = 56, 
                        created_on = '', 
                        data_type_code = '', 
                        diagnosis_lab_code = '', 
                        diagnosis_lab_name = '', 
                        document_id = 56, 
                        format_code = '', 
                        hash = '', 
                        name = '', 
                        patient_id = 56, 
                        relationship_to_proband_code = '', 
                        run_alias = '', 
                        seq_id = 56, 
                        size = 56, 
                        submitter_sample_id = '', 
                        task_id = 56, )
                    ],
        )
        """

    def testDocumentsSearchResponse(self):
        """Test DocumentsSearchResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
