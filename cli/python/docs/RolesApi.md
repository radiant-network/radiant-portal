...
Each name must be unique within the tenant in its own language.
`description_en` / `description_fr` are optional and follow the same fallback.
`actions` must list at least one action, and every one of them must exist and be grantable
a reserved action (such as `can_manage_user`) yields 422, which is what keeps it out of every custom role and
