...
can then act on the assignment. The caller comes first in the list when they are
themselves eligible, so assigning a case to oneself is the top row.
...
with radiant_python.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print("Exception when calling CasesApi->list_case_assignment_candidates: %s\n" % e)
```
...
[**List[CaseAssignee]**](CaseAssignee.md)
