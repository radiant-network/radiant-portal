...
- **Status:** Proposed
- **Date:** 2026-05-11
- **Revised:** 2026-07-28 — schema aligned to the shipped multi-tenancy model: `code` natural keys (`tenant_code`/`org_code`/`role_code`, not `*_id`), the single `public.user_role` grant table, `user_id`-keyed `public.users`, and localized (`_en`/`_fr`) role/action labels. PR-0 struck (already shipped).
...
- Each case can have **zero or more practitioner assignments** (clinicians actively working the case in the portal) managed via a join table. Unlike requester/supervisor, an assignee **must** be a portal User holding the `practitioner` role at the case's `(tenant_code, org_code)`.
...
| **Portal-active practitioners require auth** | Practitioners who *work cases inside the portal* (the assignment join) must be Users with the `practitioner` role at the case's `(tenant_code, org_code)`. Two-tier model: requester/supervisor = identity only; assignments = identity + authorization. |
...
The Practitioner table lives in the **main PostgreSQL schema** (not `auth_db`). It is a domain entity, referenced by `cases` (as requester and supervisor) and by the [case practitioner assignment join table](#case-practitioner-assignments), and optionally linked to `public.users`.

#### Prerequisite — `users` is already keyed by `user_id` (done)

The Practitioner ↔ User link uses the Keycloak `sub`. **No schema change is needed:** migration `000011` already re-keyed `public.users` so `user_id varchar(255)` is the **primary key** (email is a nullable attribute; there is no `username` column). The practitioner FK references `public.users(user_id)` directly.
...
CREATE TABLE public.practitioner (
    ...
    user_id          varchar(255) REFERENCES public.users(user_id) ON DELETE SET NULL,  -- Keycloak sub; nullable for external practitioners
    ...
    -- Composite UNIQUE so other tables can reference (id, tenant_code) as a composite FK,
...
CREATE INDEX idx_practitioner_tenant  ON public.practitioner(tenant_code);
...
| Choice | Rationale |
|---|---|
| **Per-tenant rows** (composite `UNIQUE(user_id, tenant_code)`) | A clinician working at two tenants gets two rows. Each tenant's admin owns its catalog. Data may legitimately differ (license per jurisdiction, recorded specialty, contact details). |
| **No `org_code` column** | A User who is a Practitioner can serve at multiple orgs within a tenant; their org membership is already captured by `public.user_role`. Storing org on the practitioner row would either duplicate that data or force one-row-per-org. External (non-User) practitioners simply have no org granularity recorded — acceptable, since they don't carry portal permissions. |
| **`user_id` nullable** (`varchar(255)`, Keycloak `sub`) | External (non-user) practitioners are first-class. The link stores the Keycloak `sub` and references `public.users(user_id)` — the users PK since migration `000011`. |
...
| **Composite `UNIQUE (id, tenant_code)`** | Allows other tables (`cases` for requester/supervisor, the assignment join) to reference `(id, tenant_code)` as a composite FK, structurally enforcing same-tenant linkage. Defense-in-depth: cross-tenant references fail at the DB layer, not just in the API. |
...
| Same user, same tenant, multiple orgs | ✅ — one row covers all orgs | Org membership is captured in `public.user_role`, not on the practitioner row |
| Same user, same tenant | ❌ duplicate | `UNIQUE(user_id, tenant_code)` catches it |
...
-- Composite FKs against (id, tenant_code) — guarantee the referenced practitioner
-- and the case share the same tenant_code at the database level.
ALTER TABLE public.cases
    ADD CONSTRAINT cases_requester_fk
    FOREIGN KEY (requester_id, tenant_code)
    REFERENCES public.practitioner (id, tenant_code);

ALTER TABLE public.cases
    ADD CONSTRAINT cases_supervisor_fk
    FOREIGN KEY (supervisor_id, tenant_code)
    REFERENCES public.practitioner (id, tenant_code);
```
...
> **Reconcile with existing columns:** `cases` already carries `tenant_code` (migration `000013`) plus a free-text `ordering_physician` and an `ordering_organization_code` (migrations `000001`/`000009`). `requester_id` supersedes the free-text `ordering_physician` as a structured identity — the migration should drop or deprecate `ordering_physician`, and decide whether the ordering organization is derived from the requester's practitioner or kept as `ordering_organization_code`.
...
The composite FKs on `(requester_id, tenant_code)` and `(supervisor_id, tenant_code)` enforce tenant alignment at the database level: any attempt to attach a UDN practitioner to a CBTN case (or vice versa) fails with a constraint violation, regardless of whether the API filtered correctly. This is the same defense-in-depth principle the security ADR applies to row visibility — the database is authoritative, the application is the convenience layer.

### Case practitioner assignments

A separate join table captures the set of portal-active practitioners assigned to each case. Unlike requester/supervisor, every row in this table must be a User-linked practitioner holding the `practitioner` role at the case's `(tenant_code, org_code)`.
...
CREATE TABLE public.case_practitioner_assignment (
    ...
    FOREIGN KEY (case_id, tenant_code)
        REFERENCES public.cases (id, tenant_code),
    FOREIGN KEY (practitioner_id, tenant_code)
        REFERENCES public.practitioner (id, tenant_code)
);
...
> Requires `cases` to have `UNIQUE (id, tenant_code)` (parallel to the practitioner table). 

**Eligibility rule for assignments** (invariant **I3**, API-enforced — see [Authorization Model](#authorization-model)): a row in `case_practitioner_assignment` is valid only if the referenced practitioner is **(a)** linked to a User (`practitioner.user_id IS NOT NULL`), and **(b)** that User holds the `practitioner` role at the case's `(tenant_code, org_code)` — i.e. a matching row in `public.user_role` with `role_code = 'practitioner'`, with `org_code` either matching exactly or via the `*` wildcard. External (non-User) practitioners and Users without the role at the case's org cannot be assigned. Enforced at the API layer on every INSERT.
...
The `practitioner` role is **orthogonal** to clinical-skill roles like `geneticist`. A user typically holds both: `geneticist` grants interpretation actions, `practitioner` grants case-assignment actions.
...
CRUD authority over the Practitioner catalog. Default holder: `tenant_admin` — the only tenant-level admin role in the MVP (`tenant_owner` / `tenant_manager` were dropped).

> **Seed with localized labels.** Since migration `000019`, `role` and `action` carry `name_en` / `name_fr` / `description_en` / `description_fr` (action also has `name_en` / `name_fr`). The seed for the `practitioner` role and the three new actions (`can_be_assigned_to_case`, `can_sign_report`, `can_manage_practitioner`) must populate all four label columns. Action codes follow the singular `can_*` convention (matches `can_manage_user` / `can_manage_org` / `can_manage_role`).
...
**I2 — Requester/supervisor tenant alignment.** `cases.requester_id` and `cases.supervisor_id` must reference practitioner rows in the same tenant as the case. **Structurally enforced** by the composite FKs `(requester_id, tenant_code)` and `(supervisor_id, tenant_code)` against `practitioner (id, tenant_code)`. No User-link or role required — these are identity-only relationships.

**I3 — Assignment eligibility.** Every row in `case_practitioner_assignment` must reference a practitioner that is **(a)** linked to a User (`practitioner.user_id IS NOT NULL`), and **(b)** that User holds the `practitioner` role at the case's `(tenant_code, org_code)` — matching `public.user_role.role_code = 'practitioner'`, with `org_code` either an exact match or `*`. Enforced at the API layer on INSERT (the composite FKs cover tenant alignment; user-link + role are application-level checks). External practitioners cannot be assigned.
...
Adding a practitioner to a case's assignment list (a row in `case_practitioner_assignment`) is a separate operation, performed by users with the appropriate admin permission. An authorized user opens the case, picks a practitioner from a typeahead over `practitioner` filtered to **eligible candidates only** — `active = true`, `user_id IS NOT NULL`, and the linked User holds the `practitioner` role at the case's `(tenant_code, org_code)` (specific match or `*` wildcard). External practitioners and Users without the role at the case org do not appear. The handler re-validates eligibility (invariant **I3**) before INSERT. Removal is symmetric. If the desired practitioner does not yet exist in the catalog, or lacks the role, the user contacts a `can_manage_practitioner` holder (for the record) or a `can_manage_user` holder / `tenant_admin` (for the role grant) first — assignment and provisioning remain separate flows.
...
Model Practitioner as tenant-agnostic, with a separate `practitioner_tenant` join table for tenant membership — analogous to how `users` relates to `user_role`.
...
**Cons:**
- Cross-tenant data sharing of Practitioner records contradicts the tenant-isolation philosophy of the rest of the architecture (per ADR §3 — every other PostgreSQL table is tenant-scoped via a `tenant_code` column).
...
**Verdict:** rejected in favor of `UNIQUE(user_id, tenant_code)`.
...
This work is sequenced **after** the [Security & Multi-Tenancy](./multi-tenancy-security.md) epic. Much of what this ADR originally listed as prerequisites has since shipped: `public.users` keyed by `user_id` (migration `000011`), the single `public.user_role` grant table, the `code`-keyed tenant/org/role model, and the tenant-scoped admin API for **organizations** (roles/users management endpoints are still in progress). `cases` already carries `tenant_code`. Within this practitioner epic, the only dependencies tracked below are between the `PR-X` tickets themselves.

| Ticket | Depends on | Scope |
|---|---|---|
| ~~**PR-0** — Extend users with `user_id`~~ | — | **Done** — migration `000011` already re-keyed `public.users` so `user_id varchar(255)` is the PK. No work needed. |
| **PR-1** — Practitioner table + repository | — | PostgreSQL migration (composite UNIQUEs on `(user_id, tenant_code)` and `(id, tenant_code)`; case-create lookup UNIQUE deferred to PR-6 once the resolution field is chosen); Go model + repository with `tenant_code` filtering |
...
PR-1 and PR-2 are the foundation (PR-0 is already shipped); once they're in, PR-3/4/5/6/7 fan out and can run in parallel.
...
3. **Case-create resolution mechanism for requester/supervisor.** *Major open question — blocks PR-6 schema finalization.* What identifier does the case-create API payload carry, and which `practitioner` column does the handler match against? Candidates:
   - **Email** — natural identifier, but assumes every practitioner (including external referrers) has an email on file. Would require `email` NOT NULL + `UNIQUE(email, tenant_code)`.
   ...
   Whichever is chosen needs a `UNIQUE (<field>, tenant_code)` constraint and shapes the case-create API contract. Decide before PR-6.
...
5. ~~Username vs `user_id` FK chain.~~ **Resolved** — migration `000011` already re-keyed `public.users` to `user_id` (no `username` column), and the single `public.user_role` grant table keys on `user_id`. Nothing to decide.
